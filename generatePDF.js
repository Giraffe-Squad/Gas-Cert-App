...
export function generateCertificatePDF(cert) {
  ...
  // ═══ HEADER — certificate keeps dark blue theme ═══
  doc.setFillColor(12, 31, 63);
  doc.rect(0, 0, W, 18, 'F');
  try { doc.addImage(GAS_SAFE_LOGO, 'JPEG', 3, 1.5, 15, 15); } catch (e) {}
  ...
  doc.autoTable({ startY: y, margin: { left: 12 + cw * 2 }, tableWidth: cw,
    ...
    body: mkRows([['Name', ll.name], ['Company Name', ll.company], ['Address', ll.address], ['Postcode', ll.postcode], ['Email', ll.email], ['Phone', ll.phone]]),
    ...go, columnStyles: { 0: { cellWidth: cw * 0.34 } } });

  y = doc.lastAutoTable.finalY + 2;

  // ═══ APPLIANCE TABLE — no Serial/GC ═══
