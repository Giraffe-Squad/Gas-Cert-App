      ...
      {isExpanded && (
        <div style={{ padding: "16px 18px 20px", borderTop: `1px solid ${COLORS.border}`, animation: "fadeIn 0.25s ease" }}>
          ...
          <div style={appS.grid}>
            ...
            <FormField label="Serial No." half><input style={componentStyles.input} value={appliance.serialNo} onChange={e => update("serialNo", e.target.value)} placeholder="Serial number" /></FormField>
            <FormField label="GC Number" half><input style={componentStyles.input} value={appliance.gcNumber} onChange={e => update("gcNumber", e.target.value)} placeholder="Gas Council number" /></FormField>
...
function SettingsPage({ onClose }) {
  ...
  const handleUnlock = () => { if (masterKey === MASTER_KEY) { setUnlocked(true); toast("Master key accepted", "success"); } else { toast("Invalid master key", "error"); } };
  const handleSave = () => {
    if (!newUsername.trim() || !newPassword.trim()) { toast("Both fields required", "error"); return; }
    saveToStorage("cp12_username", newUsername.trim());
    saveToStorage("cp12_password", newPassword);
    toast("Credentials updated!", "success");
    setNewUsername(""); setNewPassword(""); setUnlocked(false); setMasterKey("");
  };
        ...
        {!unlocked ? (
          <div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 16 }}>
              ...
              <input type="password" value={masterKey} onChange={e => setMasterKey(e.target.value)} placeholder="Enter master key" style={componentStyles.input} onKeyDown={e => e.key === "Enter" && handleUnlock()} />
            </div>
            <button onClick={handleUnlock} style={appS.accentBtn}><Key size={16} /> Unlock Settings</button>
          </div>
        ) : (
          <div>
            <p style={{ fontSize: 14, color: COLORS.green, fontWeight: 600, marginBottom: 16 }}>✓ Unlocked. Enter new credentials.</p>
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <button onClick={handleSave} style={appS.accentBtn}><Check size={16} /> Save Credentials</button>
            </div>
          </div>
        )}
      </div>
...
function AppContent() {
  ...
  const [currentView, setCurrentView] = useState("dashboard"); // dashboard | certificates | form
  ...
  useEffect(() => {
    const ver = "2.1";
    if (localStorage.getItem("cp12_version") !== ver) {
      ...
      localStorage.setItem("cp12_version", ver);
  ...
  // Search — MUST be before conditional returns
  ...
  const confirmDel = () => { if (!deleteTarget) return; setCertificates(prev => prev.filter(c => c.id !== deleteTarget)); setDeleteTarget(null); toast("Certificate deleted", "info"); };
  ...
  /* ═══════════════════════════════════════════════════════════════
     DASHBOARD VIEW — clean home with just Create + Completed tab
     ═══════════════════════════════════════════════════════════════ */
      ...
      <div style={appS.root}><GlobalStyles /><div style={appS.bgDots} />
        <div style={appS.container}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 32 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
              ...
              <div>
                ...
                <p style={{ fontSize: 14, color: COLORS.muted, margin: 0, marginTop: 2 }}>CP12 — Landlord Gas Safety Records</p>
              </div>
            </div>
          ...
          <button onClick={() => setCurrentView("certificates")} style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
              <div style={{ width: 48, height: 48, borderRadius: 14, background: COLORS.greenBg, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <FolderOpen size={24} color={COLORS.green} />
              </div>
              <div style={{ textAlign: "left" }}>
                <div style={{ fontSize: 17, fontWeight: 700, color: COLORS.primary }}>Completed Certificates</div>
                <div style={{ fontSize: 13, color: COLORS.muted, marginTop: 2 }}>{certificates.length} certificate{certificates.length !== 1 ? "s" : ""} saved</div>
              </div>
            </div>
  ...
  /* ═══════════════════════════════════════════════════════════════
     CERTIFICATES VIEW — large preview cards with search
     ═══════════════════════════════════════════════════════════════ */
  if (currentView === "certificates") {
    return (
      <div style={appS.root}><GlobalStyles /><div style={appS.bgDots} />
        <div style={appS.container}>
          ...
          <div style={{ position: "relative", marginBottom: 18 }}>
            ...
            <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
              placeholder="Search by address, postcode, landlord, company, or cert number..."
              style={{ ...componentStyles.input, paddingLeft: 38, fontSize: 14, padding: "12px 14px 12px 38px" }} />
          ...
          {/* Certificate List — Large Preview Cards */}
          {filteredCerts.length === 0 && (
            <div style={{ padding: 60, textAlign: "center", color: COLORS.muted, background: COLORS.card, borderRadius: 16, border: `1px solid ${COLORS.border}` }}>
              ...
              <p style={{ marginTop: 16, fontSize: 16 }}>{searchQuery ? "No certificates match your search." : "No certificates yet."}</p>
            </div>
          )}
            ...
            return (
              <div key={item.id} style={{
                ...
                <div style={{
                  ...
                  background: item.status === "complete" ? COLORS.greenBg : COLORS.amberBg,
                  borderBottom: `1px solid ${item.status === "complete" ? "#c8e6c9" : "#ffe0b2"}`,
                }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    {item.status === "complete" ? <CheckCircle2 size={16} color={COLORS.green} /> : <FileText size={16} color={COLORS.amber} />}
                    <span style={{ fontSize: 12, fontWeight: 700, color: item.status === "complete" ? COLORS.green : COLORS.amber }}>
                      {item.status === "complete" ? "Completed" : "Draft"}
                    </span>
                  </div>
                ...
                <div style={{ padding: "16px 18px" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "flex-start", gap: 10, marginBottom: 12 }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: 15, fontWeight: 600, color: COLORS.primary, marginTop: 2 }}>
                        {item.prop?.address || "No address"}{item.prop?.postcode ? ` · ${item.prop.postcode}` : ""}
                      </div>
                      {item.prop?.city && <div style={{ fontSize: 13, color: COLORS.muted }}>{item.prop.city} {item.prop.county}</div>}
                    </div>
                  </div>
                  ...
                  <div style={{ display: "flex", alignItems: "flex-start", gap: 10, marginBottom: 12 }}>
                    ...
                    <div>
                      ...
                      {item.ll?.address && <div style={{ fontSize: 12, color: COLORS.muted }}>{item.ll.address}{item.ll.postcode ? ` · ${item.ll.postcode}` : ""}</div>}
                    </div>
                  </div>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <div>
                      <div style={{ fontSize: 11, fontWeight: 600, color: COLORS.muted, textTransform: "uppercase", letterSpacing: 0.5 }}>Inspection Date</div>
                ...
                <div style={{ padding: "0 18px 16px", display: "flex", gap: 8 }}>
                  <button onClick={() => openCert(item, true)} style={{ ...appS.outBtn, flex: 1, justifyContent: "center", padding: 10, fontSize: 13 }}>
                    <Edit3 size={14} /> Edit
                  </button>
                  <button onClick={() => openCert(item, false)} style={{ ...appS.outBtn, flex: 1, justifyContent: "center", padding: 10, fontSize: 13 }}>
                    <FileText size={14} /> View
                  </button>
                  <button onClick={() => dlPDF(item)} style={{ ...appS.accentBtn, flex: 1, justifyContent: "center", padding: 10, fontSize: 13 }}>
                    <Download size={14} /> PDF
                  </button>
                  <button onClick={() => setDeleteTarget(item.id)} style={{ background: "#fff", border: `1px solid ${COLORS.border}`, borderRadius: 8, padding: "8px 10px", cursor: "pointer", color: COLORS.red, display: "flex", alignItems: "center" }}>
                    <Trash2 size={14} />
                  </button>
                </div>
              </div>
        ...
        <ConfirmDialog open={!!deleteTarget} title="Delete Certificate?" message="This certificate will be permanently deleted. This action cannot be undone." onConfirm={confirmDel} onCancel={() => setDeleteTarget(null)} />
      </div>
    );
  }

  /* ═══════════════════════════════════════════════════════════════
     FORM VIEW — 8 step wizard
     ═══════════════════════════════════════════════════════════════ */
  const renderStep = () => {
    switch (currentStep) {
      case 1: return (<div>
        <StepTitle icon={User} title="Engineer & Company Details" description="Your Gas Safe registration and business information." />
        <ChooseExisting storageKey="cp12_existing_engineers" labelField="name" secondaryField="company" onSelect={(entry) => { setActiveCert(prev => ({ ...prev, eng: { ...entry } })); toast("Engineer loaded", "success"); }} />
        <div style={appS.grid}>
          ...
          <FormField label="Telephone" half><input style={componentStyles.input} type="tel" value={cert.eng.tel} onChange={e => updateField("eng.tel", e.target.value)} placeholder="e.g. 07700 900123" /></FormField>
          <FormField label="Email" half><input style={componentStyles.input} type="email" value={cert.eng.email} onChange={e => updateField("eng.email", e.target.value)} placeholder="engineer@example.com" /></FormField>
          <FormField label="Gas Safe Reg. No." required half><input style={componentStyles.input} value={cert.eng.gasSafe} onChange={e => updateField("eng.gasSafe", e.target.value)} placeholder="e.g. 123456" /></FormField>
          <FormField label="ID Card No." half><input style={componentStyles.input} value={cert.eng.idCard} onChange={e => updateField("eng.idCard", e.target.value)} placeholder="e.g. 0012345" /></FormField>
        </div>
      </div>);
      case 2: return (<div>
        <StepTitle icon={Home} title="Inspection / Installation Address" description="Property being inspected." />
        <ChooseExisting storageKey="cp12_existing_properties" labelField="address" secondaryField="postcode" onSelect={(entry) => { setActiveCert(prev => ({ ...prev, prop: { ...entry } })); toast("Property loaded", "success"); }} />
        <div style={appS.grid}>
          <FormField label="Certificate No." half><input style={componentStyles.input} value={cert.certNo} onChange={e => updateField("certNo", e.target.value)} placeholder="e.g. 0001" /></FormField>
          ...
          <FormField label="Address" required><input style={componentStyles.input} value={cert.prop.address} onChange={e => updateField("prop.address", e.target.value)} placeholder="e.g. 12 High Street" /></FormField>
          <FormField label="Town / City" required half><input style={componentStyles.input} value={cert.prop.city} onChange={e => updateField("prop.city", e.target.value)} placeholder="e.g. London" /></FormField>
          <FormField label="County" half><input style={componentStyles.input} value={cert.prop.county} onChange={e => updateField("prop.county", e.target.value)} placeholder="e.g. Greater London" /></FormField>
          <FormField label="Postcode" required half><input style={{ ...componentStyles.input, textTransform: "uppercase", letterSpacing: 1.5, fontWeight: 600 }} value={cert.prop.postcode} onChange={e => updateField("prop.postcode", e.target.value.toUpperCase())} placeholder="e.g. SW1A 1AA" /></FormField>
          <FormField label="UPRN" half><input style={componentStyles.input} value={cert.prop.uprn} onChange={e => updateField("prop.uprn", e.target.value)} placeholder="Optional" /></FormField>
      ...
      case 3: return (<div>
        <StepTitle icon={Building2} title="Landlord / Agent / Customer" description="Details of the person responsible for the property." />
        <ChooseExisting storageKey="cp12_existing_landlords" labelField="name" secondaryField="address" onSelect={(entry) => { setActiveCert(prev => ({ ...prev, ll: { ...entry } })); toast("Details loaded", "success"); }} />
        <div style={{ display: "flex", gap: 8, marginBottom: 18 }}>
          {["landlord", "agent", "customer"].map(type => (
            <button key={type} type="button" onClick={() => updateField("ct", type)} style={{ padding: "8px 18px", borderRadius: 9, border: "1.5px solid", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit", background: cert.ct === type ? COLORS.primary : "transparent", color: cert.ct === type ? "#fff" : COLORS.muted, borderColor: cert.ct === type ? COLORS.primary : COLORS.border }}>{type.charAt(0).toUpperCase() + type.slice(1)}</button>
          ))}
        </div>
        <div style={appS.grid}>
          ...
          <FormField label="UPRN" half><input style={componentStyles.input} value={cert.ll.uprn} onChange={e => updateField("ll.uprn", e.target.value)} placeholder="Optional" /></FormField>
        </div>
      </div>);
      case 4: return (<div>
      ...
      case 6: return (<div>
        <StepTitle icon={AlertTriangle} title="Faults & Works" description="Record defects and work carried out." />
        <FormField label="Give Details of Any Faults"><textarea style={{ ...componentStyles.input, minHeight: 110, resize: "vertical", lineHeight: 1.7 }} value={cert.faults} onChange={e => updateField("faults", e.target.value)} placeholder="Describe any faults..." /></FormField>
      ...
      case 7: return (<div>
        ...
        <div style={{ background: COLORS.accentBg, borderRadius: 14, padding: 20 }}><p style={{ fontSize: 14, lineHeight: 1.7, color: COLORS.text }}>I declare that the appliance(s) and flue(s) detailed in this report have been inspected in accordance with the Gas Industry Unsafe Situations Procedure and, where relevant, the Gas Safety (Installation and Use) Regulations 1998. All readings and results recorded are an accurate record of the inspection carried out.</p></div>
      ...
      <div style={appS.container}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, cursor: "pointer" }} onClick={() => { saveCert("draft"); setCurrentView("dashboard"); }}>
            <GasSafeLogo size={44} /><div><h1 style={{ fontSize: 18, fontWeight: 800, color: COLORS.primary, margin: 0, letterSpacing: -0.5 }}>Gas Safety Record</h1><p style={{ fontSize: 11, color: COLORS.muted, margin: 0 }}>CP12 Certificate</p></div>
          </div>
      ...
      return (
        <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Outfit',sans-serif", background: COLORS.bg, padding: 20 }}>
          <div style={{ background: "#fff", borderRadius: 16, padding: 32, maxWidth: 420, width: "100%", textAlign: "center", boxShadow: "0 8px 32px rgba(0,0,0,.1)" }}>
            ...
            <p style={{ fontSize: 14, color: COLORS.muted, marginBottom: 20, lineHeight: 1.6 }}>Old data may be conflicting with the app.</p>
            ...
            <p style={{ fontSize: 11, color: COLORS.muted, marginTop: 16 }}>Error: {this.state.error?.message || "Unknown"}</p>
          </div>
        </div>
      );
    }
